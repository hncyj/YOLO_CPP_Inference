...
PreProcessResult YOLOPreProcessor::preprocess(const cv::Mat& img) {
    if (img.empty()) { throw std::invalid_argument("Input image is empty"); }
    ...
    if (result.processed_img.size() != target_size_) { throw std::runtime_error("Preprocessing failed: incorrect output size"); }
        ...
        if (max_cls_conf >= this->config_.conf_thresh) {
            ...
            for (int k = 0; k < kpt_nums; k++) {
                ...
                if (pdata_base + 2 < data_width) {
                    ...
                    throw std::out_of_range("keypoints index out of range");
                }
            }
...
{     
    if (mask_channels_ <= 0) { throw std::invalid_argument("mask_channels must be positive"); }
    if (mask_size_.width <= 0 || mask_size_.height <= 0) { throw std::invalid_argument("mask_size must be positive"); }
    if (ratio_ < 0.0f || ratio_ > 1.0f) { throw std::invalid_argument("expansion ratio must be between 0.0 and 1.0"); }
}
...
std::vector<SegmentObj> SegmentPostProcessor::decode_output(const cv::Mat& output0, const cv::Mat& output1, const cv::Vec4d& transform_params, const cv::Size& original_size,const cv::Size& target_size, bool use_NMS) {
    ...
    if (output0.cols != data_width) {
        std::cerr << "Error: Invalid output0 format. Expected " << data_width 
                  << " columns, got " << output0.cols << std::endl;
        throw std::invalid_argument("Invalid output0 format."); 
    }
